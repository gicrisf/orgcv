...
projects = [esrafel, zhuia, gotosi, unsplash]
...
*** Overview
#+name: document-latex
#+begin_src latex :noweb no-export :results output latex :export results
\begin{document}
...
\cvevent{ {{project.name}} }{ {{project.description}} }{ {{project.when}}  }
...
#+name: projects-latex
#+begin_src python :session cv-venv :results output :var templ=projects-jinja-latex :var ctx=projects-json()
...
\begin{itemize}
\item \href{ https://github.com/gicrisf/gotosi }{\faGithub gicrisf/gotosi}
\end{itemize}
\vspace{.2cm}
...
\cvevent{ {{seminario.name}} {{seminario.where}} {{seminario.when}} }
